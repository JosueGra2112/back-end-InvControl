...
if (isset($data->id) && isset($data->newStock) && isset($data->user_id)) {
    ...
    try {
        ...
        $movementStmt->bindParam(':quantity', $newStock, PDO::PARAM_INT); // Cantidad agregada
        ...
        echo json_encode(["message" => "Stock agregado correctamente"]);
    } catch (Exception $e) {
        // Revertir la transacción si ocurre un error
        $conn->rollBack();
        echo json_encode(["message" => "Error al agregar stock: " . $e->getMessage()]);
    }
} else {
